...
async function deletePersonalData(ctx: Context, confirmation: string): Promise<void> {
  ...
  if (confirmation.trim().toLowerCase() !== 'confirm') {
    await ctx.reply('Это навсегда удалит ваше резюме, поисковые настройки, оценки, решения, сохранённые отклики, статистику и настройки доставки. ' +
