...
CREATE TABLE IF NOT EXISTS candidate_prefilter_scores (
  user_id TEXT NOT NULL,
  source TEXT NOT NULL,
  source_id TEXT NOT NULL,
  context_hash TEXT NOT NULL,
  ...
  scored_at TEXT NOT NULL,
  PRIMARY KEY (user_id,source,source_id),
  FOREIGN KEY (user_id,source,source_id) REFERENCES candidate_discoveries(user_id,source,source_id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS candidate_prefilter_queue_idx ON candidate_prefilter_scores(user_id,filtered,combined_score DESC);
...
export function exportUserData(userId: string): Record<string, unknown> {
  ...
  const profileRows = db.prepare('SELECT platform,profile_json FROM search_profiles WHERE user_id=? ORDER BY platform').all(userId);
  const careerRow = profileRows.find((row) => String(row.platform) === careerProfilePlatformId);
  const searchProfiles = profileRows.filter((row) => String(row.platform) !== careerProfilePlatformId)
...
export function recordVacancyCandidate(userId: string, input: VacancyCandidateInput): boolean {
  ...
  if (hasCv(userId)) {
    ...
    if (existing && String(existing.listing_hash) !== listingHash) {
      db.prepare('DELETE FROM candidate_prefilter_scores WHERE source=? AND source_id=?').run(input.source, input.sourceId);
    }
  }
  return discovered;
}
...
export function candidatesNeedingPrefilter(userId: string, contextHash: string, limit: number): VacancyCandidate[] {
  return db.prepare(`SELECT c.* FROM vacancy_candidates c
    JOIN candidate_discoveries d ON d.source=c.source AND d.source_id=c.source_id AND d.user_id=?
    LEFT JOIN candidate_prefilter_scores p ON p.user_id=d.user_id AND p.source=c.source AND p.source_id=c.source_id
    WHERE c.status IN ('discovered','queued','filtered','failed')
    AND (p.source_id IS NULL OR p.context_hash<>? OR p.listing_hash<>c.listing_hash OR p.semantic_status='unavailable')
    ORDER BY d.last_seen_at DESC LIMIT ?`).all(userId, contextHash, limit).map(rowToCandidate);
}
export function saveCandidatePrefilter(userId: string, candidate: VacancyCandidate, contextHash: string, score: PrefilterScoreInput): void {
  db.prepare(`INSERT INTO candidate_prefilter_scores
    (user_id,source,source_id,context_hash,listing_hash,regex_score,lexical_cosine,semantic_cosine,semantic_status,
     combined_score,filtered,reasons_json,scored_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
    ...
    reasons_json=excluded.reasons_json,scored_at=excluded.scored_at`).run(userId, candidate.source, candidate.sourceId,
      contextHash, candidate.listingHash, score.regexScore, score.embeddingCosine, score.semanticCosine, score.semanticStatus,
      score.combinedScore, Number(score.filtered), JSON.stringify(score.reasons), new Date().toISOString());
}
export function rankedCandidateQueueForUsers(userIds: string[], perUserLimit: number): VacancyCandidate[] {
  const now = new Date().toISOString();
  const query = db.prepare(`SELECT c.*,p.combined_score FROM vacancy_candidates c
    JOIN candidate_discoveries d ON d.source=c.source AND d.source_id=c.source_id
    JOIN candidate_prefilter_scores p ON p.user_id=d.user_id AND p.source=c.source AND p.source_id=c.source_id
    WHERE d.user_id=? AND c.status IN ('discovered','queued','filtered','failed') AND p.filtered=0
      AND (c.next_retry_at IS NULL OR c.next_retry_at<=?)
    ORDER BY p.combined_score DESC,c.published_at DESC LIMIT ?`);
...
export function pendingVacancies(userId: string, limit: number): Vacancy[] {
  return db.prepare(`SELECT v.*,COALESCE(uv.decision,'new') user_decision FROM vacancies v
    ...
    WHERE s.vacancy_id IS NULL AND COALESCE(uv.decision,'new') NOT IN ('skipped','applied') AND EXISTS (
      SELECT 1 FROM vacancy_candidates c JOIN candidate_discoveries d ON d.source=c.source AND d.source_id=c.source_id
      WHERE c.vacancy_id=v.id AND d.user_id=?)
    ORDER BY v.published_at DESC LIMIT ?`).all(userId, userId, userId, limit).map(rowToVacancy);
}
export function vacanciesNeedingPrefilter(userId: string, contextHash: string, limit: number, semanticRequired = false): Vacancy[] {
  return db.prepare(`SELECT v.*,COALESCE(uv.decision,'new') user_decision FROM vacancies v
    ...
    AND EXISTS (SELECT 1 FROM vacancy_candidates c JOIN candidate_discoveries d
      ON d.source=c.source AND d.source_id=c.source_id WHERE c.vacancy_id=v.id AND d.user_id=?)
    ...
    ORDER BY v.published_at DESC LIMIT ?`).all(userId, userId, userId, userId, contextHash, Number(semanticRequired), limit).map(rowToVacancy);
}
