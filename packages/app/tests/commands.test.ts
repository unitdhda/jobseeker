...
function fixture() {
  ...
  const ports: CommandPorts = {
    store: { getDeliverySettings: async () => null, saveDeliverySettings: async (_user, settings) => { events.push(`window:${settings.digestHourUtc}`); },
      ...
      scoredVacancyApplyIds: async () => ['abcdef'],
      llmUsageSummary: async () => ({ turns24h: 0, turnsTotal: 0, tokens24h: 0, tokensTotal: 0,
        inputTokens24h: 0, inputTokensTotal: 0, outputTokens24h: 0, outputTokensTotal: 0,
        cacheReadTokens24h: 0, cacheReadTokensTotal: 0, cacheWriteTokens24h: 0, cacheWriteTokensTotal: 0,
    ...
    delivery: { isApprovedUser: async () => true, unsentHighScoreVacancies: async () => [], markAlerted: async () => true,
      ...
      replaceDigestSnapshot: async () => undefined, scoredVacancyApplyIds: async () => ['abcdef'] },
