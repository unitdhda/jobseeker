...
export const ru = {
  ...
  cv: {
    ...
    previewTitle: '<b>Проверьте распознанное резюме</b>',
    previewStats: (filename:string,characters:number,blocks:number) =>
      `Файл: ${filename} · ${characters.toLocaleString('ru')} символов · ${blocks} блоков`,
    previewClean: 'Ошибок распознавания не обнаружено.',
    previewWarnings: (warnings:string) => `Проверьте предупреждения распознавания: ${warnings}.`,
    previewQuestion: 'Текст похож на ваше резюме? Текущее резюме не заменится до подтверждения.',
    confirmButton: 'Использовать это резюме',
    rejectButton: 'Загрузить другое',
    confirmedToast: 'Резюме подтверждено',
    previewExpiredToast: 'Предпросмотр истёк; загрузите файл снова',
    ocrRequired: 'В PDF нет читаемого текстового слоя. Распознайте его с помощью OCR и загрузите PDF с текстом.',
