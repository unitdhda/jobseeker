...
export function scraperStatus(summary: ScraperSummary, configuredSources: readonly string[], locale: Locale,
  ...
  lines.push('', `<b>${locale === 'ru' ? 'Нормализация' : 'Normalization'}</b>`,
    `${locale === 'ru' ? 'очередь' : 'queued'}: ${formatNumber(summary.normalization.queued, locale)} · ${locale === 'ru' ? 'активные' : 'active'}: ${formatNumber(summary.normalization.activeClaims, locale)} · ${locale === 'ru' ? 'просроченные' : 'expired'}: ${formatNumber(summary.normalization.expiredClaims, locale)} · ${locale === 'ru' ? 'нечитаемые' : 'undecodable'}: ${formatNumber(summary.normalization.undecodable, locale)}`);
