...
-- A parsed upload waits here until the user confirms its preview. It expires automatically and never replaces the
-- authoritative CV merely because extraction succeeded.
create table public.pending_cv_imports (
  user_id text not null,
  cv_sha256 text not null,
  original_filename text not null,
  ...
  constraint pending_cv_imports_pkey primary key (user_id),
  constraint pending_cv_imports_user_id_fkey foreign key (user_id) references users(user_id) on delete cascade
);
...
create table public.matches (
  ...
  -- The raw combined evidence score. Semantic ordering reads prescore_score instead.
  lexical_score integer,
  -- Free lexical evidence exactly as it stood at match time, retained for diagnostics.
  lexical_regex_score integer,
  lexical_cosine double precision,
  -- The two signals lexical_regex_score collapses together, retained separately for diagnostics.
  ...
  -- The rarity-aware evidence, both 0..1. Null means no vocabulary existed when the row was written.
  lexical_specificity double precision,
  lexical_cosine_idf double precision,
  -- Cheap semantic judge used for live admission and ranking.
  prescore_score integer,
  prescore_model text,
  prescore_prompt_version integer,
  prescore_updated_at timestamptz,
  -- Frozen when the prescore lands. Selected rejects buy unbiased quality labels.
...
-- vacancies during daily matching-vocabulary maintenance, so rows are derived data.
--
