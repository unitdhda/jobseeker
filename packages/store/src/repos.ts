...
export async function linkStoreVacancies(userId: string, limit: number, roleTerms: readonly string[]): Promise<StoreLinkResult> {
  ...
  const candidates = await q(`insert into ${columns}
    ...
    order by ${relevance} desc,v.published_at desc nulls last limit $3
    on conflict do nothing returning source_id`, [userId, timestamp, limit, terms, storeSettings().searchPlatforms, freshest]);
...
export function sourceQuotaFor(perUserLimit: number, sourceCount: number): number {
  if (storeSettings().normalizationPerSourceQuota > 0) return storeSettings().normalizationPerSourceQuota;
...
export async function rankedCandidateQueueForUsers(userIds: string[], perUserLimit: number): Promise<VacancyCandidate[]> {
  await ready();
  const quota = sourceQuotaFor(perUserLimit, storeSettings().searchPlatforms.length);
  // The pool is wide enough to hold every source's quota plus overflow candidates for the leftover slots.
  const poolLimit = perUserLimit + quota * storeSettings().searchPlatforms.length;
  const queues = await Promise.all(userIds.map(async (userId) => {
    const rows = await q(`select * from (select c.*,d.search_name discovery_search_name,
      ...
      [userId,now(),poolLimit,storeSettings().searchPlatforms]);
...
export async function saveScore(userId:string,vacancyId:number,score:number,primaryTrack:string,summary:string,reasons:string[],gaps:string[],hardRejection:boolean):Promise<void>{
  await ready(); await withPostgresTransaction(async client=>{if(!await hasCv(userId,client))throw new Error('An authoritative CV source is required.');
    ...
    const alert=score>=storeSettings().alertScore&&!hardRejection&&String(rows[0]?.decision??'new')==='new';
