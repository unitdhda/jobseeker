...
Команды владельца `/users`, `/revoke` и `/usage` доступны вручную, но не добавляются в меню Telegram.
...
|---|---|---|
| `OPENAI_CODEX_AUTH_FILE` | Записываемый OAuth JSON для провайдера | обязательна |
| `FLUE_MODEL` | Модель Flue | указана в `.env.example` |
| `FLUE_THINKING_LEVEL` | Глубина рассуждений модели | `high` |
...
Контейнер работает без root, с read-only файловой системой, сброшенными capabilities и доступом к порту только через loopback deployment host.

## Обновление deployment host

Из корня проекта на локальном Mac:
...
SSH_PORT=22 REMOTE_DIR='~/hh-jobseeker' \
  ./scripts/deploy-deployment host.sh user@deployment-host
```

Скрипт загружает исходники и конфигурацию, пересобирает Compose-сервис, сохраняет существующий volume и ждёт успешный health check. Перед запуском убедитесь, что локальная копия бота не опрашивает Telegram: одновременно должен работать только один poller.
